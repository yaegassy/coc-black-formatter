...
async function doInstall(pythonCommand: string, context: ExtensionContext): Promise<void> {
  ...
  try {
    window.showInformationMessage(`Installing black-formatter language server in progress...`);
    await exec(installCmd);
    statusItem.hide();
    window.showInformationMessage(`Installation of black-formatter language server is now complete!`);
  } catch (error) {
    statusItem.hide();
    window.showErrorMessage(`Installation of black-formatter language server failed. | ${error}`);
    throw new Error();
  }
}
